...
class TosaTestGen:
    ...
    def serialize(self, testName, metaData=None, tags=None, testRequirements=None):
        ...
        if metaData and self.args.lazy_data_gen:
            # Only produce CPP files in lazy data gen mode
            # NOTE: CPP files are examples - and will be changed in future
            if "data_gen" in metaData:
                # Output data generation meta data as CPP data
            ...
            if "compliance" in metaData:
                # Output compliance meta data as CPP data
    ...
    def tensorComplianceMetaData(
        ...
        if (
            ...
            # RESCALE special case
            mode = gtu.ComplianceMode.RESCALE_INEXACT
            compliance_tens["rescale_inexact_info"] = {
                "unsigned_data": argsDict["output_unsigned"]
            }
        elif not gtu.dtypeIsFloat(outputTensor.dtype):
            # Integer EXACT
            ...
            # compute ABS_BOUND & VARIANCE_ERROR_BOUND based on types
            if inputType in (DType.FP8E5M2, DType.FP8E4M3) and acc_dtype == DType.FP32:
                # The FP8 dot product with FP32 accumulator has relaxed bounds
                abs_bound = 2 * max(ksb, min(ksb, 64) * (1 << 10))
                variance_bound = 4 * 0.4 * max(ksb, min(ksb, 64) * (1 << 20))
            else:
                abs_bound = 2 * ksb
    ...
    def build_avg_pool2d(
        ...
        attr = ts.TosaSerializerAttribute()
        attr.AvgPool2dAttribute(kernel, stride, pad, accum_dtype)
        self.ser.addOperator(op["op"], input_list, output_list, attr)
    ...
    TOSA_OP_LIST = {
        ...
        "avg_pool2d": {
            ...
            "build_fcn": (
                build_avg_pool2d,
                TosaTensorGen.tgAvgPool2D,
                TosaTensorValuesGen.tvgAvgPool2D,
        ...
        "mul": {
            ...
            "error_if_validators": (
                ...
                TosaErrorValidator.evDimensionMismatch,
                TosaErrorValidator.evBroadcastShapesMismatch,
                TosaErrorValidator.evInputRank1WrongRank,
            ),
        ...
        "rescale": {
            ...
            "error_if_validators": (
                ...
                TosaErrorValidator.evWrongInputList,
                TosaErrorValidator.evWrongOutputList,
                TosaErrorValidator.evRescaleInputUnsignedOutputUnsigned,
                TosaErrorValidator.evRescaleI32OutputInputUnsigned,
                TosaErrorValidator.evRescaleI32InputOutputUnsigned,
                TosaErrorValidator.evRescaleI48InputOutputUnsigned,
