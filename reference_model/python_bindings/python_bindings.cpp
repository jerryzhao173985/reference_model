...
struct IModelRunnerPyWrapper
{
    ...
    int setInput(std::string input_name, py::array input)
    {
        ...
        // This case is specific to v0.80 and earlier, since in later versions
        // the un-templated version of IModelRunner.setInput supports more dtypes
        tosa::TosaSerializationTensor* ser_tensor =
            handler.GetMainRegion()->GetBlocks()[0]->GetTensorByName(input_name);
        if (ser_tensor->GetDtype() == tosa::DType_INT8)
        {
            int8_t* typed_ptr = static_cast<int8_t*>(info.ptr);
            std::vector<int32_t> padded(typed_ptr, typed_ptr + info.size);
            return runner.setInput(input_name, padded);
        }
    ...
    py::array getOutputBytes(std::string output_name)
    {
        ...
        // This case is specific to v0.80 and earlier, since in later versions
        // the un-templated version of IModelRunner.getOutput supports more dtypes
        if (ser_tensor->GetDtype() == tosa::DType_INT8)
        {
            std::vector<int32_t> out = runner.getOutput<int32_t>(output_name);
            if (out.empty())
            {
                return {};
            }
            py::array_t<int32_t> bytes(out.size(), out.data());
                                                         ...
                                                         string tosa_level)
{
    ...
    func_config.tosa_profile                          = tosa_profile;
...
verbosity : str, default='NONE'
                            ...
                            R"(
tosa_profile : int, default=1
    TOSA profile (0 = Base Inference, 1 = Main Inference, 2 = Main Training)
...
ValueError
    ...
    m.def("run", &run_model, docstring.c_str(), "tosa_binary"_a, "inputs"_a, py::kw_only(), "inst_id"_a = 0,
          ...
          "tosa_profile"_a = 1, "dump_intermediates"_a = 0, "initialize_variable_tensor_from_numpy"_a = 0,
          "fp_format"_a = "0.5", "precise_mode"_a = 0, "abs_mode"_a = false, "tosa_level"_a = "NONE");
}
