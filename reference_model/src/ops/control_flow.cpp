                             ...
                             std::vector<TosaReference::Tensor*>& block_outputs)
{
    ...
    for (size_t i = 0; i < static_cast<size_t>(num_input_tensors); i++)
    {
    ...
    for (size_t i = 0; i < static_cast<size_t>(num_output_tensors); i++)
    {
        const TosaReference::Tensor* ct = block_sgt.getOutputTensor(static_cast<uint32_t>(i));
        ASSERT_MEM(ct);
        if (!ct->getIsValid())
        {
...
int OpCondIf<Rank>::checkTensorAttributes()
{
    ...
    auto else_region = tsh->GetRegionByName(attribute->else_graph());
    ...
    {
        // old serialization: then_graph and else_graph point to blocks in curr_region
        auto region_name = getParentSGT()->getRegionName();
        auto curr_region = tsh->GetRegionByName(region_name);
        then_block       = curr_region->GetBlockByName(attribute->then_graph());
        else_block       = curr_region->GetBlockByName(attribute->else_graph());
    }

    ERROR_IF(!then_block, "OpCondIf: fail to resolve then_graph %s", attribute->then_graph().c_str());

    ERROR_IF(!else_block, "OpCondIf: fail to resolve else_graph %s", attribute->else_graph().c_str());
    ...
    ERROR_IF(then_block->GetInputs().size() != num_input_tensor, "OpCondIf: then_block has unexpected number of input");
    ERROR_IF(else_block->GetInputs().size() != num_input_tensor, "OpCondIf: else_block has unexpected number of input");
    ERROR_IF(then_block->GetOutputs().size() != num_output_tensor,
             "OpCondIf: then_block has unexpected number of output");
    ERROR_IF(else_block->GetOutputs().size() != num_output_tensor,
    ...
    for (size_t i = 0; i < num_input_tensor; i++)
    {
        ...
        for (size_t d = 0; d < static_cast<size_t>(operator_input->getRank()); d++)
        {
    ...
    for (size_t i = 0; i < num_output_tensor; i++)
    {
        ...
        for (size_t d = 0; d < static_cast<size_t>(operator_output->getRank()); d++)
        {
...
int OpCondIf<Rank>::eval()
{
    ...
    if (cond_val)
    {
        ...
        {
            WARNING("OpCondIf: Fail to evaluate then branch block %s", attribute->then_graph().c_str());
            return 1;
        }
    }
    else
    {
        ...
        {
            WARNING("OpCondIf: Fail to evaluate else branch block %s", attribute->else_graph().c_str());
            return 1;
        }
    }
...
int OpWhileLoop::checkTensorAttributes()
{
    ...
    else
    {
        auto region_name = getParentSGT()->getRegionName();
        auto curr_region = tsh->GetRegionByName(region_name);
        cond_block       = curr_region->GetBlockByName(attribute->cond_graph());
        body_block       = curr_region->GetBlockByName(attribute->body_graph());
    }

    ERROR_IF(!cond_block, "OpWhileLoop: fail to resolve cond_graph %s", attribute->cond_graph().c_str());
    ...
    ERROR_IF(getOutputs().size() != num_block_tensor, "OpWhileLoop: operator input tensor doesn't match output");
    ERROR_IF(cond_block->GetInputs().size() != num_block_tensor,
             "OpWhileLoop: cond_block has unexpected number of input");
    ERROR_IF(body_block->GetInputs().size() != num_block_tensor,
             "OpWhileLoop: body_block has unexpected number of input");
    ERROR_IF(body_block->GetOutputs().size() != num_block_tensor,
...
int OpWhileLoop::eval()
{
    ...
    while (eval_count++ < MAX_WHILE_LOOP_ITERATION)
    {
        ...
        {
            WARNING("OpWhileLoop: Fail to evaluate cond block %s", attribute->cond_graph().c_str());
            return 1;
        }
        ...
        if (cond_val)
        {
            ...
            {
                WARNING("OpWhileLoop: Fail to evaluate body block %s", attribute->body_graph().c_str());
                return 1;
            }
