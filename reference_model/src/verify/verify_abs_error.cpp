...
ErrorBoundsRange calcErrorBounds(double referenceValue, double boundsValue, const void* cfgPtr)
{
    ...
    ASSERT_MSG(cfg != nullptr, "AbsErrorVerifyInfo is nullptr in calcErrorBounds");
    ...
    double bValue = cfg->maxCompare > 0.0 ? std::max(errorBound, (cfg->maxCompare)) : errorBound;
                    ...
                    const AbsErrorVerifyInfo& aeInfo)
{
    ...
    switch (implementationTensor->data_type)
    {
        ...
        case tosa_datatype_bf16_t: {
            ...
            TOSA_REF_REQUIRE(impData != nullptr, "[AE] Missing data for implementation");
            return validateData(refData, refBndData, impData, refShape, modeStr, &aeInfo, &calcErrorBounds<bf16>);
        }
        case tosa_datatype_fp8e4m3_t: {
            const auto* impData = reinterpret_cast<const fp8e4m3*>(implementationTensor->data);
            TOSA_REF_REQUIRE(impData != nullptr, "[AE] Missing data for implementation");
            return validateData(refData, refBndData, impData, refShape, modeStr, &aeInfo, &calcErrorBounds<fp8e4m3>);
        }
        case tosa_datatype_fp8e5m2_t: {
            const auto* impData = reinterpret_cast<const fp8e5m2*>(implementationTensor->data);
            TOSA_REF_REQUIRE(impData != nullptr, "[AE] Missing data for implementation");
            return validateData(refData, refBndData, impData, refShape, modeStr, &aeInfo, &calcErrorBounds<fp8e5m2>);
        }
        default:
